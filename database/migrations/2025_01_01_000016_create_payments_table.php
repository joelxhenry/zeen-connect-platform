...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('payments', function (Blueprint $table) {
            ...
            $table->enum('payment_type', ['full', 'deposit', 'balance'])->default('full');
            $table->enum('processing_fee_payer', ['client', 'provider'])->nullable();

            // Gateway details
            $table->string('gateway')->default('wipay');
            $table->string('gateway_type')->default('escrow');
            $table->string('gateway_provider')->nullable();
            $table->string('gateway_transaction_id')->nullable();
            $table->string('gateway_order_id')->nullable();
            $table->string('gateway_response_code')->nullable();
            $table->text('gateway_response')->nullable();
            $table->string('split_transaction_id')->nullable();
