...
const form = useForm({
    account_number: props.config?.merchant_account_id ?? '',
    api_key: '',
...
const submit = () => {
    if (props.isEdit) {
        form.put(`/payments/setup/wipay`, {
            preserveScroll: true,
        });
    } else {
        form.post(`/payments/setup/wipay`, {
            preserveScroll: true,
        });
    }
};
...
<template>
    <ConsoleLayout :title="isEdit ? 'Edit WiPay' : 'Set Up WiPay'">
        <div class="w-full max-w-2xl mx-auto">
            <!-- Page Header -->
            <ConsolePageHeader
                :title="isEdit ? 'Edit WiPay Configuration' : 'Set Up WiPay'"
                :subtitle="isEdit ? 'Update your WiPay account credentials' : 'Connect your WiPay account to receive payments'"
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    <ConsoleFormSection>
                        <div>
                            ...
                            />
                            <small v-if="form.errors.account_number" class="text-red-500">
                                {{ form.errors.account_number }}
                            </small>
                            <small class="text-gray-500 block mt-1">
                                Found in your WiPay dashboard under Account Settings
                            </small>
                        </div>
                    </ConsoleFormSection>
            ...
            <!-- Features Info -->
            <ConsoleFormCard title="What you get with WiPay" icon="pi pi-info-circle" class="mt-6">
